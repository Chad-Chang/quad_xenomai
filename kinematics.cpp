...
Vector2f Kinematics ::get_posRW_error(int idx) // idx = 0이면 현재 값, idx = 1이면 이전 값
{

  Vector2f RWpos_error;
  Vector2f RWpos_error_old;
...
Vector2f Kinematics ::get_velRW_error(int idx) // idx = 0이면 현재 값, idx = 1이면 이전 값
{

  Vector2f RWvel_error;
  Vector2f RWvel_error_old;
...
void Kinematics:: model_param_cal(double thm, double thb)
{
  ...
  double th2 = thb - thm;
   /* Trunk Parameters */
    ...
    Lamda_nominal_FOB(0,0) = M1; // RW task space inertia 
    ...
    // result for DOB -> uncoupled model 
    ...
    Lamda_nominal_DOB = JacobianTrans*Inertia_DOB*Jacobian; // biarticular joint space
}
